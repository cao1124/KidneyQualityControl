...
def get_loss(model, images, texts, text_tags, loss_img, loss_txt, args, accum_image_features=None, accum_text_features=None, accum_idx=-1, teacher_model=None, teacher_accum_image_features=None):
    ...
    ground_truth = torch.eye(logits_per_image.shape[0]).to(torch.float32)
    # for i in range(len(text_tags)-1):
    #     for j in range(i+1, len(text_tags)):
    #         if text_tags[i] == text_tags[j]:
    #             ground_truth[i][j] = 1
    #             ground_truth[j][i] = 1
    index_map = defaultdict(list)
    for index, value in enumerate(text_tags):
        k = value.item()
        index_map[k].append(index)    
    index_map = dict(index_map)

    for index, value in enumerate(text_tags):
        k = value.item()
        element_positions = index_map[k]   
        if len(element_positions)>1:
            for i in range(len(element_positions)-1):
                for j in range(i+1, len(element_positions)):
                    ground_truth[element_positions[i]][element_positions[j]] = 1
                    ground_truth[element_positions[j]][element_positions[i]] = 1
    ...
    if args.report_training_batch_acc:
        ...
        i2t_acc = ground_truth[range(len(ground_truth)), logits_per_image.argmax(-1)].sum() / len(logits_per_image)
        t2i_acc = ground_truth[range(len(ground_truth)), logits_per_text.argmax(-1)].sum() / len(logits_per_text)
...
def train(model, data, epoch, optimizer, scaler, scheduler, args, global_trained_steps, teacher_model=None):
    ...
    for i in range(0, dataloader.num_batches):
        ...
        images, texts, eos_indices, text_tags = batch  # yksin change
        ...
        if args.accum_freq == 1:
            ...
            if args.precision == "amp":
                with autocast():
                    if args.distillation:
                        total_loss, acc = get_loss(model, images, texts, text_tags, loss_img, loss_txt, args, teacher_model=teacher_model)
                    else:
                        total_loss, acc = get_loss(model, images, texts, text_tags, loss_img, loss_txt, args)
...
def evaluate(model, data, epoch, args, steps):
    ...
    with torch.no_grad():
        for i in range(dataloader.num_batches):
            ...
            # cumulative_i2t_acc += ((logits_per_image.argmax(-1) == ground_truth).sum()).float()  # yksin change
            # cumulative_t2i_acc += (logits_per_text.argmax(-1) == ground_truth).sum().float()
            cumulative_i2t_acc += (ground_truth[range(len(ground_truth)), logits_per_image.argmax(-1)].sum()).float()  # yksin change
            cumulative_t2i_acc += (ground_truth[range(len(ground_truth)), logits_per_text.argmax(-1)].sum()).float()
