...
def main():
    ...
    os.environ['MASTER_PORT'] = '8131' # '8131'
    ...
    # args.device = torch.device("cuda:5", args.local_device_rank)
    ...
    args.log_path = os.path.join(args.logsdir, args.name, "out_{}.log".format(time_suffix))

    args.checkpoint_path = os.path.join(args.logsdir, args.name, "checkpoints")
    if is_master(args):
    ...
    if is_master(args):
        logging.info("Params:")
        params_file = os.path.join(args.logsdir, args.name, "params_{}.txt".format(time_suffix))
    ...
    # determine if this worker should save logsdir and checkpoints.
    # only do so if it is the 0th worker.
    args.should_save = (args.logsdir is not None and args.logsdir != '' and args.logsdir.lower() != 'none') and is_master(args)
